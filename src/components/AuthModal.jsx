...
export default function AuthModal({ isOpen, onClose }) {
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    // Minimal mock validation: allow login if email/mobile filled and password length >= 6
    if ((!emailRegex.test(loginEmail) && !mobileRegex.test(loginEmail)) || loginPassword.length < 6) {
      setLoginError('Enter valid email/mobile and password (6+ chars)');
      return;
    }
      ...
      <div className="px-2">
        ...
        <div className="flex justify-center mb-4">
          <div className="inline-flex bg-gray-100 rounded-full p-1">
            <button type="button" className={`px-4 py-1 text-sm rounded-full ${tab==='login' ? 'bg-white shadow font-semibold' : ''}`} onClick={() => setTab('login')}>Log In</button>
            <button type="button" className={`px-4 py-1 text-sm rounded-full ${tab==='signup' ? 'bg-white shadow font-semibold' : ''}`} onClick={() => setTab('signup')}>Sign Up</button>
          </div>
        </div>
        ...
        {tab === 'login' ? (
          <form onSubmit={handleLogin} className="space-y-3">
            ...
            <button type="submit" className="w-full px-4 py-2 bg-gray-200 text-gray-500 rounded" disabled={(!emailRegex.test(loginEmail) && !mobileRegex.test(loginEmail)) || loginPassword.length < 6}>Log In</button>
          ...
          <form onSubmit={handleSignup} className="space-y-3">
            ...
            <div>
              ...
              <input className={`w-full border rounded px-3 py-2 ${errors.mobile? 'border-red-500':'border-gray-200'}`} value={mobile} onChange={(e)=>setMobile(e.target.value.replace(/\D/g,''))} placeholder="0000000000" />
            ...
            <div>
              <label className="block text-sm mb-1">Password <span className="text-red-500">*</span></label>
              <input type="password" className={`w-full border rounded px-3 py-2 ${errors.password? 'border-red-500':'border-gray-200'}`} value={password} onChange={(e)=>setPassword(e.target.value)} />
            ...
            <button type="submit" className="w-full px-4 py-2 bg-gray-200 text-gray-500 rounded" disabled={!name || !mobileRegex.test(mobile) || !emailRegex.test(email) || password.length<6 || password!==confirmPassword}>Sign Up</button>
            <div className="border-t my-2" />
